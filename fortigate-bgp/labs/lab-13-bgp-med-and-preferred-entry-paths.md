...
R2 and R3 do not need a BGP session with each other for this exercise. Each router independently carries routes between R1 and R4.
...
172.18.12.1 4      65023      10      11        2    0    0 00:03:12        2
172.18.13.1 4      65023      10      11        2    0    0 00:03:09        2
...
<strong>                        set set-metric 200
        ...
        config router bgp
            config neighbor
                edit "172.18.13.0"
                    set route-map-out "RM_MED_R1_OUT"
                next
            end
        end
    next
...
Paths: (1 available, best #1, table Default-IP-Routing-Table)
  ...
  65023 65004
    172.18.12.1 from 172.18.12.1 (2.2.2.2)
      Origin IGP distance 20 metric 200, localpref 100, valid, external, best
```
...
## Exercise 6: Remove the MED Policy

Remove the outbound route-maps from the R1 neighbors on R2 and R3:
...
Both paths should return to metric 0. With the important attributes equal again, BGP will use its later tie breakers to select one best path.
